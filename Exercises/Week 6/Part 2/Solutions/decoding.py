...
class GreedySearchDecoderForCausalLM(GeneratorForCausalLM):
    ...
    def search(
        ...
        for t in range(max_new_tokens):
            ...
            # score += torch.max(log_probs).item()
        ...
        #-------------------------------------------------
        # # USE CACHE VERSION
        # # Prepare the inputs as well as their attention masks
        # model_inputs = self.prepare_next_inputs(model_inputs=inputs, use_cache=True)
        # # score = 0.0
        
        # final_out = model_inputs["input_ids"].tolist()
        ...
        #     final_out[0].append(new_token_id.item())
        ...
        # # Return the sequence generated
        # return torch.tensor(final_out).to(device=model_inputs["input_ids"].device)
        ...
        for t in range(max_new_tokens):
            ...
            for b in beams:
                ...
                if b["model_inputs"]["input_ids"][0][-1] == self.eos_token_id:
                    ...
                    logits = outputs.logits
                    log_probs = torch.nn.functional.log_softmax(logits[0, -1], dim=-1)
                    topk_log_probs, topk_tokens = torch.topk(input=log_probs, k=num_beams)
                    ...
                    for log_prob, new_token_id in zip(topk_log_probs, topk_tokens):
                        ...
                        new_beam.update({'length_penalized_score': new_beam['score'] / (tot_length ** length_penalty)})
