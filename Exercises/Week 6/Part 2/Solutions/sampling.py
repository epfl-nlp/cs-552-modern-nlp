...
class TopKSamplerForCausalLM(GeneratorForCausalLM):
    ...
    def sample_helper(self, logits, k, temperature):
        ...
        values, indices = torch.topk(logits / temperature, k, largest = True, sorted = True)
    ...
    def sample(
        ...
        for t in range(max_new_tokens):
            ...
            new_token_id = self.sample_helper(logits[0,-1], top_k, temperature)
    ...
    def sample(
        ...
        for _ in range(max_new_tokens):
            ...
            with torch.no_grad():
                outputs = self.model(**model_inputs)
            logits = outputs.logits
            new_token_id = self.sample_helper(logits[0,-1], top_p, temperature)
